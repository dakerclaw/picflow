...
数据不需要做任何迁移，服务端启动时会自行收拾历史遗留问题。但有几件事会变，需要知道：

| 变化 | 影响 | 你要做什么 |
|------|------|-----------|
| JWT 密钥不再使用公开默认值 | 所有已签发的令牌失效，用户需要重新登录（本站登录只需用户名，基本无感）；分享链接会失效，重新分享即可 | 什么都不用做。想自己管密钥就设 `JWT_SECRET` |
| `settings.json` 语义变了 | 它现在只存 4 个公开设置，且**只补缺失的键**，不会再覆盖数据库里的值 | 旧备份文件里的闸门密钥会被忽略（不影响闸门本身，它读数据库） |
| settings 表里的历史脏数据 | 旧版本每次全新安装都会写入 7 行 `[NULL, NULL]`，新版本启动时会自动清掉并补上正确的默认值 | 自动完成，日志里会打印「清理了 N 行无主设置」 |
| 上传改为按文件头校验 | 之前伪装成图片的文件（若有）不再能上传；已经躺在 `uploads/` 里的非图片文件会变成「只能下载」，不会再被浏览器当作页面执行 | 想彻底清掉就手工删掉 `uploads/` 里非图片的文件 |
| `/api/settings` 返回值变少 | 只返回站名/标题/图标/版权 4 个键 | 前端和后台页面用的就是这 4 个，无需改动 |
...
| `JWT_SECRET` | 自动生成 | JWT 签名密钥。**留空即可**，服务端会自己生成一个随机密钥并存到 `data/.jwt-secret`；只有想自己管理密钥时才填 |
| `JWT_SECRET_FILE` | `<数据目录>/.jwt-secret` | 自动生成的密钥存放位置 |
| `UPLOAD_DIR` | `./uploads` | 图片存储目录 |
| `DB_PATH` | `./picflow.db` | SQLite 数据库路径 |
| `SETTINGS_JSON_PATH` | `<数据目录>/settings.json` | 公开设置的备份文件路径 |
| `TRUST_PROXY` | 关闭 | 反向代理支持。挂了 nginx/Caddy 等反代就设成 `1`；容器端口能被人直连则保持关闭。详见表下说明 |

**关于 `JWT_SECRET`**：这个密钥负责签发站点访问令牌、分享令牌和用户登录态 —— 拿到它就等于拿到整站的万能钥匙（可以自己签一个令牌直接跳过访问密码）。所以：

- 代码里**没有**内置任何默认密钥。留空时服务端会在首次启动生成 96 位十六进制随机串，保存到 `data/.jwt-secret`（权限 0600），之后每次启动都复用它。
- 如果显式传入的是 `picflow-production-secret`、`change-me-to-a-random-string` 这类公开示例值，服务端会**拒绝采用**并打印警告，转而使用自动生成的那个 —— 因为这种值等于没设。
- 请把数据目录（Docker 里是 `data` 卷）纳入备份。删掉 `.jwt-secret` 只会让所有人重新登录一次，不会丢数据。

**关于 `TRUST_PROXY`**：默认关闭时，服务端只认 TCP 连接的真实来源 IP。这带来两个方向的影响：

- 前面挂了反向代理却不开它 → 所有访客会被算成同一个 IP，密码连续输错 10 次会把**全站**锁 10 分钟；
- 端口能被公网直连却开了它 → 任何人伪造一个 `X-Forwarded-For` 就能无限次试密码，限速形同虚设。

所以按实际情况显式声明，别猜。

## ⚙️ 安全说明

以下几件事在默认配置下就已经做好了，不需要额外操作：

| 项目 | 行为 |
|------|------|
| 上传文件类型 | **只认文件头字节**，不认客户端声明的 MIME，也不认文件名后缀。伪装成 `image/png` 的 HTML、SVG、纯文本一律拒绝并清理落盘文件 |
| 支持的图片格式 | JPG / PNG / GIF / WebP / BMP / AVIF。SVG 属于「可执行文档」，为了杜绝同源脚本执行，刻意不支持 |
| 落盘文件名 | 服务端随机 UUID + 由**内容检测结果**决定的扩展名，与用户上传的文件名无关 |
| `/uploads` 响应 | 一律 `X-Content-Type-Options: nosniff` + CSP `sandbox`；只有白名单扩展名按图片内联返回，其余强制 `application/octet-stream` + 附件下载。另带 `Cross-Origin-Resource-Policy: same-origin`，站外页面引用本站图片会被浏览器拦下（相当于防盗链） |
| 设置接口 | 只读写的 4 个公开键（站名/标题/图标/版权），闸门密码哈希等键不在白名单内，读写都被丢弃 |
| 密码存储 | 站点访问密码用 bcrypt 哈希，明文不落库；备份文件 `settings.json` 里也不含任何密钥 |
| 登录态 | 每次请求都会重新查一次数据库，**封禁立即生效**（不是等 7 天令牌过期） |
| 数据库落盘 | 先写临时文件再原子替换，进程在写入途中被杀不会留下半截数据库；`SIGTERM` 时主动落盘 |

### 已知风险（当前有意保留）

**用户登录只校验用户名，不校验密码。** 也就是说：知道站点访问密码的人，只要输入某个账号的用户名（包括第一个注册的管理员账号），就能以那个身份登录。这是产品层面的取舍，不是漏洞遗留 —— 请务必**开启全局访问密码**，把访问控制交给它。

> 补充：前端注册/登录表单里其实已经有「密码」输入框，注册时会把密码提交并存成 bcrypt 哈希 —— 只是登录接口目前不使用它（前端登录请求也只带用户名）。将来要收紧的话，前端只需把密码一起提交，后端加一次 `bcrypt.compareSync` 即可。
...
| 方法 | 路径 | 说明 | 鉴权 |
|------|------|------|------|
| POST | `/api/auth/register` | 注册（表单里的密码会存成 bcrypt 哈希） | - |
| POST | `/api/auth/login` | 登录。**只校验用户名**，不校验密码 —— 见「安全说明 → 已知风险」 | - |
...
**关于「不是有效的图片」**：服务端判定图片类型时只读文件的前几个字节，不看文件名、
也不看请求里声明的类型。所以：

- 把 `.html` / `.svg` / 文本文件改名成 `.png` 再上传，会被拒绝，并明确告诉你哪些文件不合格（整批拒绝，不会一半成功一半失败）；
- 真实图片即使文件名后缀写错（例如 PNG 存成 `.jpg`）也照样能上传，落盘时会按真实类型改成 `.png`；
- 支持 JPG / PNG / GIF / WebP / BMP / AVIF，**不支持 SVG**（它是可执行文档，能在浏览器里跑脚本）。
